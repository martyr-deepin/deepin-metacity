                            ...
                            XWindowAttributes *attrs)
{
  ...
  if (window && window->type == META_WINDOW_DESKTOP) {
      ...
      fprintf(stderr, "%s redirect desktop (0x%x)\n", __func__, xwindow);
                          ...
                          XEvent     *event)
{
   ...
   * keybinding is used.
   */
  XIEvent* input_event = meta_display_get_input_event(window->display, event);
  XIEnterEvent* enter_ev = (XIEnterEvent*)input_event;

  Window evwindow = event->xany.window;
  int ty = event->type;
  int mode = event->xfocus.mode;
  int detail = event->xfocus.detail;
  if (enter_ev) {
      if (input_event->evtype == XI_FocusIn) ty = FocusIn;
      else if (input_event->evtype == XI_FocusOut) ty = FocusOut;
      evwindow = enter_ev->event;
      mode = enter_ev->mode;
      detail = enter_ev->detail;
  }

  meta_topic (META_DEBUG_FOCUS,
              "Focus %s event received on %s 0x%lx (%s) "
              "mode %s detail %s\n",
              (enter_ev && ty == FocusIn) ? "in (xi)" :
              (enter_ev && ty == FocusOut) ? "out (xi)" :
              ty == FocusIn ? "in" :
              ty == FocusOut ? "out" :
              ty == UnmapNotify ? "unmap" :
              "???",
              window->desc, event->xany.window,
              evwindow == window->xwindow ?
              "client window" :
              (window->frame && evwindow == window->frame->xwindow) ?
              "frame window" :
              "unknown window",
              ty != UnmapNotify ?
  ...
  else if (ty == FocusOut || ty == UnmapNotify)
    {
      if (ty == FocusOut && detail == NotifyInferior)
        {
	  ...
	  if (elapsed < 200.0)
	    {
	       ...
	       * greater than 200, so we can disable sync
	       */
	      if (remaining)
		*remaining = 200.0 - elapsed + 100;
                                        ...
                                        XIEvent* input_event)
{
  ...
  switch (input_event->evtype)
    ...
    case XI_ButtonRelease:
          ...
          if (meta_grab_op_is_moving (window->display->grab_op))
            {
              ...
              else if (dev->root == window->screen->xroot)
                update_move (window, dev->mods.effective & ShiftMask,
                             dev->root_x, dev->root_y);
            }
          else if (meta_grab_op_is_resizing (window->display->grab_op))
            {
              if (dev->root == window->screen->xroot)
                update_resize (window,
                               dev->mods.effective & ShiftMask,
                               dev->root_x,
                               dev->root_y,
                               TRUE);
              if (window->display->compositor)
      ...
      meta_display_end_grab_op (window->display, dev->time);
      break;
